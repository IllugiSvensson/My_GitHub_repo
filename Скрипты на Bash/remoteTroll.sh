...
ListItem="zenity --title=\"Восстановление репозитория\" --height=350 --width=350 --list --text=\"Выберите доступный компьютер.\" --column=\"Имя\" --column=\"Описание\" --ok-label=\"Применить\" --cancel-label=\"Выход\""
...
ssh $answer killall RepositoryTroll.bin
ssh $answer launch grn-reposcheck &
sleep 1s
pid=`ssh $answer pgrep RepositoryTroll.bin`
