...
[ -z "$1" ] && {		#Проверяем аргумент продукта, вызываем справку

	zenity --warning --ellipsize --text="Не указан продукт в конфигурации.\nСмотри справку в консоли." --title="Предупреждение" --timeout=5 &>/dev/null
...
function PingHosts {			#Проверка хостов в сети
	...
	fi | zenity --progress --title="Выполнение" --text="Ищем компьютеры.." --pulsate --no-cancel --auto-close
...
function SoftSync {				#Проверка и синхронизация софта
	...
	[ $ver1 $2 $ver2 ] && {							#Сравниваем файл версии
		...
		} | zenity --progress --title="Выполнение" --text="Передача файлов.." --pulsate --no-cancel --auto-close
		...
		[ $flg == 0 ] && {					#Если нашли адрес, запрещаем обработку
			...
			do
				...
				do
					...
					sleep 1s
					) | zenity --progress --title="Выполнение" --text="Проверяем интерфейс $eth.." --pulsate --no-cancel --auto-close
