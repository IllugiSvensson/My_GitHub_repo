...
#Вызываем справку по первому или второму аргументу
[ "$1" == "--help" -o "$2" == "--help" ] && {
	...
	echo "	1. Проверить /system/etc/system.xml графу Config. Убедиться,"
	echo "		что в /soft/etc и /system/etc есть такой конфиг."
	echo "		Для Oracle8 каталога /system нет, проверяем /soft/etc."
	echo " "
	echo "	2. Отредактировать ZIP.xml в /system/etc/CONFIG."
	echo "		Для Oracle8 /soft/etc/CONFIG/hardware."
	echo "		В ZIP.xml в графе NetConf добавить продукт, указанный"
	echo "		в system.xml и набор адресов:"
	echo "		<n n=........ -> адрес, который всегда доступен(в сети)"
	echo "		v=........ /> -> адрес, который назначится ЗИПу."
	echo "		Соответственно оба адреса должны быть в одной подсети."
	echo "		Добавить пару адресов компьютеров, которые всегда в сети."
	echo " "
	echo "	3. Проверяем запуск программы через launch. Убедиться,"
	echo "		что launch changews_remote стартует без ошибок. Если не"
	echo "		стартует, занести запуск в объектовый launch.xml,"
	echo "		обязательно указав в графе parameters продукт из ZIP.xml"
	echo "		В блоке Hosts объектового launch.xml в графе ZIP прописываем"
	echo "		changews_remote для автозапуска приложения."
	echo " "
	echo "	4. Отредактировать Executables и Modes в /soft/etc/CONFIG/PRODUCT."
	echo "		Добавляем запуск приложения changews_remote через задачи."
	echo " "
	echo "	5. Обязательно распространить настройки и скрипт на все компьютеры,"
	echo "		включая идущие в поставку ЗИПы."
	echo " "
	echo " Скрипт должен настраиваться и применяться отдельно для каждого продукта"
	echo " и версии ОСи. В противном случаее есть вероятность сломать софт."
	echo " Чтобы в список продуктов не попадали лишние строки, нужно в"
	echo " соответствющем PRODUCT.xml в графе UseGenConf поставить 0."
	echo " "
	exit 0
...
function MakeDialogWindow {		#Создание диалогового окна
	...
	ListItem="zenity --title=\"Смена роли рабочего места\" --height=350 --width=350 --list --text=\"$1\" $2 --ok-label=\"Применить\" --cancel-label=\"Выход\""
...
function PingHosts {			#Проверка хостов в сети
	...
	fi | zenity --progress --title="Проверка сети" --text="Ищем компьютеры в сети.." --pulsate --no-cancel --auto-close
...
function DevConfig {			#Настройка железа
	...
	do
		...
		then
			...
			MakeDialogWindow "Проведите настройку оборудования.\nЧтобы задать роль, нажмите Выход." "--column=\"Оборудование\"" "Звук Диски" $DATA/CWR_online.txt
			...
			MakeDialogWindow "Проведите настройку оборудования.\nЧтобы задать роль, нажмите Выход." "--column=\"Оборудование\"" "Мониторы Звук Диски" $DATA/CWR_online.txt
			case `cat $DATA/CWR_online.txt` in
				"Мониторы")
					$DISPLAYCONF
					;;
		...
		[ $flg == 0 ] && {					#Если нашли адрес, запрещаем обработку
			...
			do
				...
				do
					...
					sleep 1s
					) | zenity --progress --title="Проверка сети" --text="Проверяем интерфейс $eth.." --pulsate --no-cancel --auto-close
