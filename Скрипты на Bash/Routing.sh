...
then
	...
	EXT_IP="$2"	#Адрес шлюза в сети, которую нужно получить   192.168.199.72
	INT_IP="$3"	#Адрес шлюза в сети, по которой пойдет трафик 192.168.10.72/24
	LAN_IP="$4"	#Адрес цели, которая будет получать трафик    192.168.10.12
...
then
	...
	EXT_IP="$2"	#не нужен, поставить нолик			0
	INT_IP="$3"	#Сеть, которую нужно получить на клиенте 	192.168.199.0/24
	LAN_IP="$4"	#адрес клиента					192.168.10.72
