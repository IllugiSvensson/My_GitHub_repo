...
then
	...
	echo "Проверить, что уже включен: sysctl -a | grep net.ipv4.ip_forward"
	echo " "
	...
	echo "Хост, который транслирует трафик становится невидимым по той сети, которую"
	echo "транслирует, но сам он видит всех участников транслируемой сети"
	echo "При работе по бродкасту скрипт автоматически завершится через 12 часов, а не"
	echo "после разрыва соединения с хостом приёмника"
	echo "-------------------------------------------------------------------------------"
	exit 0
...
[ $? == 0 ] && {
	...
	then
		...
		N=`iptables -S | egrep $4 | grep -n "$EXT_ETH -o $INT_ETH" | cut -d : -f 1`
		iptables -D FORWARD $N &>/dev/null
		flag=0; p=$4; t=5s

		[ `echo $4 | egrep -o '\w+$'` == 255 ] && {	#Если адрес бродкастовый

			p=127.0.0.1	#Переключаем пинг на себя, чтобы цикл работал

		}

		while [ `ping -c 1 -w 10 $p &>/dev/null; echo $?` == "0" ]	#Пока адрес приемника доступен
		do
			...
			then
				...
				flag=1
				[ `echo $4 | egrep -o '\w+$'` == 255 ] && {	#Условие выхода из цикла
		...
		echo " "
		N=`iptables -S | egrep $4 | grep -n "$EXT_ETH -o $INT_ETH" | cut -d : -f 1`
		iptables -D FORWARD $N &>/dev/null #Вылезают ошибки, когда отключаем интерфейс
	...
	then
		...
		route del -net $2 netmask $4 gateway $3 $ETH &>/dev/null
		flag=0
