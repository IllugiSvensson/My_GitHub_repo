...
void MainWindow::on_openButton_clicked() {
    ...
    QString s = QFileDialog::getOpenFileName(this, "Заголовок окна",
        ...
        if (s.length() > 0) { //Проверяем файл и его расширение
            ...
            if (file.open(QFile::ReadOnly | QFile::ExistingOnly)) {
                ...
                if (index != -1 && s.length() - 4 == index) { //если текстовый файл
                    ...
                    QDataStream stream(&file);  //Проверка файла на MagicHead..
                    ...
                    uint amount = 0; //Загружаем словарь и восстанавливаем текст
    ...
    if (s.length() > 0) { //Проверяем файл и его расширение
        ...
        if (file.open(QFile::WriteOnly)) {
            ...
            if (index != -1) {         //если текстовый файл (выполняются
                ...
                QHash<QChar, int>symbamount; //Посчитаем, сколько раз символы
                symbamount.clear();          //встречаются в тексте
                ...
                for (int i = 0; i < amount; ++i) {
                    //Сортируем по убыванию встречаемости в тексте
                    QChar chi = usedS[i];
                ...
                //**********************************************
                b = usedS.toUtf8();
                amount = b.length();        //Количество байт в словаре
                stream.writeRawData(reinterpret_cast<char*>(&amount), sizeof amount);
                stream.writeRawData(b.data(), amount); //Строка словаря
                //***********************************************
                amount = str.length();  //Сохраняем текст в последовательность индексов
                for (int i = 0; i < amount; i++) {
